...
function Edit({
  ...
  const renderForm = () => (
    <form className='control' onSubmit={event => handleEditForm(event)}>
      ...
      <label className='edit-label'>Edit Your Subscriptions:</label> <br />
      {topicState.topics && topicState.subs ? (
        <TopicForm breaks='true' />
      ) : null}
      <br />
      <input className='button is-link' type='submit' value='Submit Changes' />
      <br />
      <br />
      <button className='button is-danger del-btn' onClick={handleDeleteUser}>
        Delete Account
      </button>
    </form>
  )
